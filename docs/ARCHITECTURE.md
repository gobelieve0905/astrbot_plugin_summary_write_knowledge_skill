...
`directory.py` 更新本插件内存中的 `config.schema`，使用 AstrBot 4.28.0 原生 list/options/labels 多选控件，不修改核心或前端。后台每 300 秒读取平台实例、原生库目录、飞书可见群及成员，分页与请求均有限时。仅缓存身份目录；权限失败保留已知与已选 ID。旧知识库文本规范化为列表，唯一名称匹配转换为 ID，磁盘配置在用户保存时持久化。

## 限定来源与原生技能读取（2026-09-15）

`SourceWindow` 为每次请求冻结来源目录与读取记录。长话题需先读取再选取，范围必须包含在已读字符区间内，保存校验只接收被选片段和原始请求。不得用模型提供的历史或文件路径替换实际来源。原始快照按指纹保留，选取范围进入版本来源；短话题沿用完整上下文。引用仍由可信话题绑定确定，选取不改变 ACL 或项目绑定。

`native_skills.py` 复用原生 SkillManager、插件配置过滤和当前人格解析；每次读操作重新核对技能是否启用及人格可见性。只允许已安装目录内的普通 UTF-8 文本，禁止绝对路径、目录穿越、符号链接、隐藏文件和非文本扩展。读取不授予 Shell 或文件写入权限。
